    ...
    def train(self):
        ...
        try:
            ...
            if pretrained and Path(pretrained).exists():
                logger.info(f"使用预训练权重: {pretrained}")
                model = RTDETR(pretrained)
            else:
            ...
            results = model.train(
                ...
                lr0=train_config.get('lr0', 0.0001),
            ...
            logger.info(f"\n2. 评估模型:")
            logger.info(f"   python eval_rtdetr.py --model {results.save_dir / 'weights' / 'best.pt'}")
            logger.info(f"\n3. 推理测试:")
            logger.info(
                f"   python predict_rtdetr.py --model {results.save_dir / 'weights' / 'best.pt'} --source test.jpg")
            logger.info("=" * 60)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="RT-DETR Training Script for RoboMaster",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    # 基础训练
    python train_rtdetr.py --config .../rtdetr_l.yaml

    # 从检查点恢复
    python train_rtdetr.py --config configs/rtdetr_l.yaml --resume .../last.pt

    # 打印配置后退出
    python train_rtdetr.py --config .../rtdetr_l.yaml --print-config
        """
    )
