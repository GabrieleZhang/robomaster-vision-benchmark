...
class YOLO26Trainer:
    ...
    def __init__(self, config_path):
        ...
        logger.info("=" * 60)
        logger.info("\n💡 YOLO26 核心特性:")
        logger.info("  ✓ 端到端NMS-Free推理")
        logger.info("  ✓ CPU推理速度提升43%")
        logger.info("  ✓ MuSGD优化器（来自LLM领域）")
        logger.info("  ✓ Progressive Loss Balancing")
        logger.info("  ✓ Small-Target-Aware Label Assignment")
        logger.info("=" * 60)
    ...
    def _validate_config(self):
        ...
        if not torch.cuda.is_available():
            logger.warning("CUDA不可用，将使用CPU训练（较慢）")
        else:
    ...
    def train(self):
        ...
        try:
            ...
            logger.info(f"使用MuSGD优化器（YOLO26特色）")
            ...
            results = model.train(
                ...
                # 优化器配置（YOLO26默认使用MuSGD）
                optimizer=train_config.get('optimizer', 'auto'),  # YOLO26自动使用MuSGD
                ...
                # 性能优化
                amp=train_config.get('amp', True),  # 混合精度训练
            ...
            logger.info("\n" + "=" * 60)
            logger.info("YOLO26 优势总结:")
            logger.info("=" * 60)
            logger.info(" NMS-Free端到端推理 - 延迟更低更稳定")
            logger.info(" CPU推理速度快43% - 适合边缘设备")
            logger.info(" MuSGD优化器 - 收敛更快更稳定")
            logger.info(" ProgLoss + STAL - 小目标检测更好")
            ...
            logger.info(f"\n2. 评估模型:")
            logger.info(f"   python eval_yolo26.py --model {results.save_dir / 'weights' / 'best.pt'}")
            logger.info(f"\n3. 测试端到端推理（无NMS）:")
            logger.info(f"   python predict_yolo26.py --model {results.save_dir / 'weights' / 'best.pt'} --e2e")
            logger.info("=" * 60)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="YOLO26 Training Script for RoboMaster (2025 Latest Release)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    # 基础训练
    python train_yolo26.py --config .../yolo26_s.yaml

    # 从检查点恢复
    python train_yolo26.py --config configs/yolo26_s.yaml --resume .../last.pt

    # 打印配置后退出
    python train_yolo26.py --config .../yolo26_s.yaml --print-config
...
def main():
    ...
    # 创建训练器
    trainer = YOLO26Trainer(args.config)
